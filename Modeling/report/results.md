...
Four robust takeaways emerge from our findings:
...
4. **Analytical closed-form MVO provides massive computational speedups.** End-to-end IPO training requires differentiating through the portfolio 
   optimization layer thousands of times per epoch. By restricting our 
   framework to equality constraints and deriving the closed-form analytical 
   MVO solution in pure PyTorch, we bypassed numerical solver bridges like 
   `cvxpylayers`. In an isolated benchmark (batch size 64, 100 assets), our 
   analytical layer executed in **0.30 seconds**, compared to **17.4 seconds** for the numerical equivalent—a **58.6x speedup**. This architectural 
   optimization makes training complex nonlinear models tractable without 
   massive bottlenecks.
